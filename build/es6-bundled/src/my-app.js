...
</custom-style>`;document.head.appendChild($_documentContainer$6.content);const $_documentContainer$7=document.createElement("template");$_documentContainer$7.setAttribute("style","display: none;");$_documentContainer$7.innerHTML=`<dom-module id="paper-icon-button">
  <template strip-whitespace="">
    <style>
      :host {
        display: inline-block;
...
</dom-module>`;document.head.appendChild($_documentContainer$7.content);Polymer({is:"paper-icon-button",hostAttributes:{role:"button",tabindex:"0"},behaviors:[PaperInkyFocusBehavior],properties:{src:{type:String},icon:{type:String},alt:{type:String,observer:"_altChanged"}},_altChanged:function(newValue,oldValue){var label=this.getAttribute("aria-label");if(!label||oldValue==label){this.setAttribute("aria-label",newValue)}}});const bundledImportMeta$2=babelHelpers.objectSpread({},meta,{url:new URL("../node_modules/plastic-image/plastic-image.js",meta.url).href});class PlasticImage extends customElements.get("iron-image"){static get is(){return"plastic-image"}static get template(){if(!window.PlasticImageTemplate){window.PlasticImageTemplate=customElements.get("iron-image").template.cloneNode(!0)}return window.PlasticImageTemplate}static get importMeta(){return bundledImportMeta$2}static get properties(){return{srcset:{type:String},_selectedImgUrl:{type:String,notify:!0},delayLoad:{type:Boolean,value:!1,notify:!0,reflectToAttribute:!0},useElementDim:{type:Boolean,reflectToAttribute:!0,value:!1},fallbackSrc:{type:String,reflectToAttribute:!0},lazyLoad:{type:Boolean,value:!1},_lazyLoadPending:{type:Boolean,value:!0},_browserSupportsWebp:{type:Boolean,value:!1},_webpRegex:{type:RegExp,value:/.+\.webp$/i},webpRegex:{type:String,observer:"_webpRegexObserver"},_hasWebp:{type:Boolean,value:!1}}}static get observers(){return["_doImageSelectionObserver(fallbackSrc, srcset, delayLoad, _lazyLoadPending, _webpRegex)","_cleanupWillChange(fade, loaded, src)"]}connectedCallback(){super.connectedCallback();this.preventLoad=!0;if(this.fade){this.$.placeholder.style.willChange="opacity"}if(this.lazyLoad){this._initLazyLoad()}else{this._lazyLoadPending=!1;if(!this.delayLoad){this.assignImgSrc()}}}disconnectedCallback(){super.disconnectedCallback();if(this.lazyLoad&&this._lazyLoadPending&&window.plasticImageIntersectionObserver){window.plasticImageIntersectionObserver.observer.unobserve(this);if(0<window.plasticImageIntersectionObserver.counter){window.plasticImageIntersectionObserver.counter--}}}_doImageSelectionObserver(fallbackSrc,srcset,delayLoad,lazyLoadPending){if(!delayLoad&&!lazyLoadPending){this.assignImgSrc()}}_cleanupWillChange(fade,loaded,src){if(!fade||!loaded||!src){return}timeOut.run(()=>idlePeriod.run(()=>{this.$.placeholder.style.willChange=""}),500)}assignImgSrc(){let srcArray=this.srcset&&1<this.srcset.length?this.srcsetParse(this.srcset):[];this._hasWebp=0<srcArray.filter(srcitem=>{return this._webpRegex.test(srcitem.url)}).length;this._checkBrowserWebpSupport(srcArray).then(arr=>{beforeNextRender(this,()=>{this._selectedImgUrl=0<srcArray.length?this.bestMatchingImage(arr):this.fallbackSrc;if(!this.src||this.src!==this._selectedImgUrl){this.src=this._selectedImgUrl}this.preventLoad=!1})})}deepUnique(arr){return arr.sort().filter((el,i)=>JSON.stringify(el)!==JSON.stringify(arr[i-1]))}srcsetParse(input){function isSpace(c){return" "===c||"\t"===c||"\n"===c||"\f"===c||"\r"===c}function collectCharacters(regEx){var chars,match=regEx.exec(input.substring(pos));if(match){chars=match[0];pos+=chars.length;return chars}}var inputLength=input.length,regexLeadingSpaces=/^[ \t\n\r\u000c]+/,regexLeadingCommasOrSpaces=/^[, \t\n\r\u000c]+/,regexLeadingNotSpaces=/^[^ \t\n\r\u000c]+/,regexTrailingCommas=/[,]+$/,regexNonNegativeInteger=/^\d+$/,regexFloatingPoint=/^-?(?:[0-9]+|[0-9]*\.[0-9]+)(?:[eE][+-]?[0-9]+)?$/,url,descriptors,currentDescriptor,state,c,pos=0,candidates=[];while(!0){collectCharacters(regexLeadingCommasOrSpaces);if(pos>=inputLength){return candidates}url=collectCharacters(regexLeadingNotSpaces);descriptors=[];if(","===url.slice(-1)){url=url.replace(regexTrailingCommas,"");parseDescriptors()}else{tokenize()}}function tokenize(){collectCharacters(regexLeadingSpaces);currentDescriptor="";state="in descriptor";while(!0){c=input.charAt(pos);if("in descriptor"===state){if(isSpace(c)){if(currentDescriptor){descriptors.push(currentDescriptor);currentDescriptor="";state="after descriptor"}}else if(","===c){pos+=1;if(currentDescriptor){descriptors.push(currentDescriptor)}parseDescriptors();return}else if("("===c){currentDescriptor=currentDescriptor+c;state="in parens"}else if(""===c){if(currentDescriptor){descriptors.push(currentDescriptor)}parseDescriptors();return}else{currentDescriptor=currentDescriptor+c}}else if("in parens"===state){if(")"===c){currentDescriptor=currentDescriptor+c;state="in descriptor"}else if(""===c){descriptors.push(currentDescriptor);parseDescriptors();return}else{currentDescriptor=currentDescriptor+c}}else if("after descriptor"===state){if(isSpace(c)){}else if(""===c){parseDescriptors();return}else{state="in descriptor";pos-=1}}pos+=1}}function parseDescriptors(){var pError=!1,w,d,h,i,candidate={},desc,lastChar,value,intVal,floatVal;for(i=0;i<descriptors.length;i++){desc=descriptors[i];lastChar=desc[desc.length-1];value=desc.substring(0,desc.length-1);intVal=parseInt(value,10);floatVal=parseFloat(value);if(regexNonNegativeInteger.test(value)&&"w"===lastChar){if(w){pError=!0}if(0===intVal){pError=!0}else{w=intVal}}else if(regexFloatingPoint.test(value)&&"x"===lastChar){if(d){pError=!0}if(0>floatVal){pError=!0}else{d=floatVal}}else if(regexNonNegativeInteger.test(value)&&"h"===lastChar){if(h){pError=!0}if(0===intVal){pError=!0}else{h=intVal}}else{pError=!0}}if(!pError){candidate.url=url;if(w){candidate.width=w}if(d){candidate.density=d}if(h){candidate.height=h}candidates.push(candidate)}else if(console&&console.log){console.log("Invalid srcset descriptor found in '"+input+"' at '"+desc+"'.")}}}srcsetStringify(arr){return arr.map(el=>{if(!el.url){throw new Error("URL is required.")}let ret=[el.url];if(el.width){ret.push(el.width+"w")}if(el.height){ret.push(el.height+"h")}if(el.density){ret.push(el.density+"x")}return ret.join(" ")}).join(", ")}bestMatchingImage(arr){let densityMultiplier=1,hasDensity=!1;arr.forEach(src=>{if("density"in src){hasDensity=!0}});if(!hasDensity){densityMultiplier=window.devicePixelRatio||1}let refDim=this.useElementDim?{width:(this.clientWidth||window.innerWidth||document.documentElement.clientWidth)*densityMultiplier,height:(this.clientHeight||window.innerHeight||document.documentElement.clientHeight)*densityMultiplier,density:!hasDensity?1:window.devicePixelRatio||1}:{width:(window.innerWidth||document.documentElement.clientWidth)*densityMultiplier,height:(window.innerHeight||document.documentElement.clientHeight)*densityMultiplier,density:!hasDensity?1:window.devicePixelRatio||1},filtered=this.fallbackSrc?arr.concat([{url:this.fallbackSrc}]):arr;filtered=this.filterLarge(this.filterLarge(this.filterLarge(filtered,"width",refDim.width),"height",refDim.height),"density",refDim.density);filtered=this.filterSmall(this.filterSmall(this.filterSmall(filtered,"width"),"height"),"density");return filtered[0].url}filterLarge(arr,attr,refVal){if(2>arr.length)return arr;let largest=!1;for(let i=0;i<arr.length;i++){if(arr[i][attr]){if(!largest||largest[attr]<arr[i][attr]){largest=arr[i]}}}if(!largest)return arr;let filtered=[];for(let i=0;i<arr.length;i++){if(!arr[i][attr]||arr[i][attr]>=refVal)filtered.push(arr[i])}if(0==filtered.length)filtered.push(largest);return filtered}filterSmall(arr,attr){if(2>arr.length)return arr;let smallest=!1;for(let i=0;i<arr.length;i++){if(arr[i][attr]){if(!smallest||smallest[attr]>arr[i][attr]){smallest=arr[i]}}}if(!smallest)return arr;let filtered=[];for(let i=0;i<arr.length;i++){if(!arr[i][attr]||arr[i][attr]<=smallest[attr])filtered.push(arr[i])}return filtered}_initLazyLoad(polyfilled=!1){if(!("IntersectionObserver"in window)){let polyfillScript=document.getElementById("polyfill-IntersectionObserver");if(!polyfillScript){polyfillScript=document.createElement("script");polyfillScript.id="polyfill-IntersectionObserver";polyfillScript.src=this.importPath+"intersection-observer.js";polyfillScript.async=!0;document.head.appendChild(polyfillScript)}polyfillScript.addEventListener("load",()=>this._initLazyLoad(!0))}else{if(!window.plasticImageIntersectionObserver){window.plasticImageIntersectionObserver={counter:0,observer:new IntersectionObserver(entries=>{entries.forEach(entry=>{entry.target._lazyLoadCallback(entry)})},{})};if(polyfilled){window.plasticImageIntersectionObserver.observer.POLL_INTERVAL=120}}window.plasticImageIntersectionObserver.observer.observe(this);window.plasticImageIntersectionObserver.counter++}}_lazyLoadCallback(e){if(this._lazyLoadPending&&(e.isIntersecting||.001<=e.intersectionRatio)){this._lazyLoadPending=!1;if(window.plasticImageIntersectionObserver&&window.plasticImageIntersectionObserver.observer){window.plasticImageIntersectionObserver.observer.unobserve(this);if(0<window.plasticImageIntersectionObserver.counter){window.plasticImageIntersectionObserver.counter--}}}}_checkBrowserWebpSupport(srcArray){return new Promise(resolve=>{if(!this._hasWebp){resolve(srcArray)}else{if(window.plasticImageWebp){this._supportsWebp=window.plasticImageWebp.supports;resolve(this._filterWebp(srcArray,window.plasticImageWebp.supports))}else{let webpImg=new Image;webpImg.onload=()=>{let result=0<webpImg.width&&0<webpImg.height;window.plasticImageWebp={supports:result};this._supportsWebp=result;resolve(this._filterWebp(srcArray,result))};webpImg.onerror=()=>{window.plasticImageWebp={supports:!1};this._supportsWebp=!1;resolve(this._filterWebp(srcArray,!1))};webpImg.src="data:image/webp;base64,UklGRhoAAABXRUJQVlA4TA0AAAAvAAAAEAcQERGIiP4HAA=="}}})}_filterWebp(srcArray,supports){return srcArray.filter(srcitem=>{return this._webpRegex.test(srcitem.url)===supports})}_webpRegexObserver(n){if(n){let regParts=n.match(/^\/(.*?)\/([gim]*)$/);if(regParts){this._webpRegex=new RegExp(regParts[1],regParts[2])}else{this._webpRegex=new RegExp(n,"i")}}else{this._webpRegex=/.+\.webp$/i}}}window.customElements.define(PlasticImage.is,PlasticImage);const bundledImportMeta$3=babelHelpers.objectSpread({},meta,{url:new URL("../node_modules/plastic-resize-aware/plastic-resize-aware.js",meta.url).href});class PlasticResizeAware extends PolymerElement{static get template(){return html`
      <style>
        :host {
          display: block;
        }
      ...
      <app-drawer-layout fullbleed="" narrow="{{narrow}}">
        ...
        <app-drawer id="drawer" slot="drawer" swipe-open="[[narrow]]">
          ...
          <iron-selector selected="[[page]]" attr-for-selected="name" class="drawer-list" role="navigation">
            <a name="view1" href="[[rootPath]]view1">plastic-aspect-ratio<br>
            <span style="font-size:smaller;">basic demo</span></a>
            <a name="view2" href="[[rootPath]]view2">plastic-resize-aware<br>
            <span style="font-size:smaller;">basic demo</span></a>
            <a name="view4" href="[[rootPath]]view4">plastic-element-query<br>
            <span style="font-size:smaller;">Change style based on element size</span></a>
            <a name="view3" href="[[rootPath]]view3">plastic-image<br>
            <span style="font-size:smaller;">basic demo</span></a>
            <a name="view-about" href="[[rootPath]]view-about">About<br>
            <span style="font-size:smaller;">About the demo project</span></a>
          </iron-selector>
          <div class="drawer-list">
            <a name="view3" href="https://mlisook.github.io/plastic-map-info">plastic-map-info<br>
              <span style="font-size:smaller;">demo site</span></a>
            <a name="view3" href="https://mlisook.github.io/plastic-map-marker-svg">plastic-map-marker-svg<br>
              <span style="font-size:smaller;">demo site</span></a>
            <a name="view3" href="https://mlisook.github.io/paper-input-place">paper-input-place<br>
              <span style="font-size:smaller;">demo site</span></a>
          </div>
        </app-drawer>
        ...
        <app-header-layout has-scrolling-region="">
          ...
          <app-header slot="header" condenses="" reveals="" effects="waterfall">
            <app-toolbar>
              ...
              <div main-title="">plastic-element-collection Demonstrations</div>
            </app-toolbar>
          </app-header>
